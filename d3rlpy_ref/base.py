...
class LearnableBase:
    ...
    def fitter_stage1(
        ...
        if n_epochs is None and unlearn_steps is not None:
            ...
            LOG.debug("RandomIterator is selected.")
            print("RandomIterator is selected.")
            ...
            LOG.debug("RoundIterator is selected.")
            print("RoundIterator is selected.")
        else:
        ...
        # training loop
        total_step = 0
        first_unlearn_pending = start_with_unlearn
        for epoch in range(1, remain_n_epochs + 1):
            ...
            def run_unlearn_updates() -> None:
                print('Unlearning step: ')
                ...
                last_unlearn_itr = -1
                for unlearn_itr in unlearn_range_gen:
                    batch = next(unlearn_iterator)
                    loss = self.update_stage1_unlearn(batch, alpha)
                    last_unlearn_itr = unlearn_itr

                # Record metrics for the completed D_f block.
                for name, val in loss.items():
                    logger.add_metric(name, val)
                    unlearn_epoch_loss[name].append(val)

                if last_unlearn_itr % 10 == 0:
                    mean_loss = {
            ...
            for itr in range_gen:
                ...
                if first_unlearn_pending:
                    run_unlearn_updates()
                    first_unlearn_pending = False
                ...
                # Unlearn after each retain block, or before the first retain update
                # when start_with_unlearn is enabled.
