...
class CRR(AlgoBase):
    ...
    def _update_stage1_unlearn(
        self, batch: TransitionMiniBatch, alpha
    ) -> Dict[str, float]:
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR
    ...
    def _update_stage2(
        self, batch: TransitionMiniBatch, original_algo: AlgoBase
    ) -> Dict[str, float]:
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR

        critic_loss = self._impl.update_critic_unlearn(
            batch, algo=original_algo
        )
        actor_loss = self._impl.update_actor(batch)
