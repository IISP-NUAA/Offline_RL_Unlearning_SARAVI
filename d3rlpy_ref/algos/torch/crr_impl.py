...
class CRRImpl(DDPGBaseImpl):
    ...
    @train_api
    @torch_api()
    def update_actor_unlearn(
        self, batch: TorchMiniBatch, alpha
    ) -> np.ndarray:
    ...
    @train_api
    @torch_api()
    def update_critic_unlearn(
        self, batch: TorchMiniBatch, **kwargs
    ) -> np.ndarray:
        ...
        original_q_tpn = algo._impl.compute_target(batch)
        current_q_tpn = self.compute_target(batch)

        loss = (
            0.1 * self.compute_critic_loss(batch, original_q_tpn.detach())
            + self.compute_critic_loss(batch, current_q_tpn)
        )
