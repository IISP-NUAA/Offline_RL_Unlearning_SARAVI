...
#  Parses KL/Wasserstein JSONs, extracts Shuffle Mode, scales metrics by 100, and generates Final Summary.
...
try:
    ...
    #  Fallback if the other file isn't in path
...
def parse_kl_record(data: Dict[str, Any], file_path: str) -> Dict[str, Any]:
    ...
    # 1.  Extract Metadata
    unlearn_model_dir = data.get("unlearn_model_dir", "")
    method_name, ratio, algo_name, _ = parse_model_dir(unlearn_model_dir)
    row["Method Name"] = method_name
    row["Algo. Name"] = algo_name
    row["Shuffle Mode"] = get_shuffle_mode(unlearn_model_dir) 
    step = parse_step_from_model_dir(unlearn_model_dir)
    if step == "N/A":
    ...
    if "kl_statistics" in data:
        ...
        metric_type = "JS"
    elif 'critic_divergence_statistics' in data:
        stats = data.get("critic_divergence_statistics", {})
        metric_type = "critic_divergence"
    else:
...
def main():
    ...
    if not results_df.empty:
        ...
        # =================================================================
        #  NEW: Scale targeted metric columns by 100, excluding 'count'
        # =================================================================
        cols_to_scale = [col for col in existing_stat_cols if "count" not in col.lower()]
        ...
        if numeric_cols:
            ...
            # =================================================================
            #  Generate Ratio Summary (Aggregated over Seed, split by Ratio)
            # =================================================================
            ...
            # =================================================================
            # 6.  Generate Final Summary (Aggregated over Ratio & Seed)
            # =================================================================
