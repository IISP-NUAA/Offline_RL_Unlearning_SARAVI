...
def generate_output_filename(args, root_path):
    ...
    base_name = (
        f"run_finetune_only_{args.algo}_{root_name}_"
...
def main():
    ...
    for model_info in found_models:
        ...
        for (unc_file, ratios) in param_combinations:
            ...
            cmd = (
                f"python fine_tune_on_remain_set.py --dataset {args.dataset} \\{newline}"
        ...
        if max_jobs <= 1:
            ...
            for i, cmd in enumerate(commands):
                ...
                if is_batch_full or is_last_command:
                    ...
                    f.write(f"echo 'Waiting for {len(pids)} jobs in batch {i // max_jobs + 1} to finish...'\n")
