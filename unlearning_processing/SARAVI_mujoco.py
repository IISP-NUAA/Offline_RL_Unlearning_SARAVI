...
def compute_adaptive_entropy_based_penalty(
    ...
    quantiles = torch.linspace(0, 1, 32, device=q_values.device)
    ...
    loss_shape = torch.nn.functional.mse_loss(q_vals_f, q_vals_r)
    # loss_shape = torch.nn.functional.l1_loss(q_vals_f, q_vals_r)
    loss_scale_brake = (torch.log(std_f) - torch.log(std_r)).abs()
...
def custom_unlearning_loop(
    ...
    if 'BCQ' in filter:
        args.batch_size = round(args.batch_size*2)
        batch_size_r = prior_batch_size
    else: 
    ...
    total_steps = round(args.unlearning_steps*0.5)
    ...
    for step in pbar:
        ...
        if 'TD3' not in filter:
            total_critic_loss = original_critic_loss + critic_penalty_on_Df
        ...
        # 9. --- Saving ---
        if step % args.eval_interval == 0 or step % total_steps == 0:
...
def run_unlearning(args: argparse.Namespace):
    ...
    # uncertainty_f_raw = np.load(uncertainty_f_path) 
    # print("loaded: ", uncertainty_f_path)
    # if len(transitions_f) != len(uncertainty_f_raw):
    #     raise ValueError("FATAL MISMATCH! Forget transitions != uncertainty file.")
